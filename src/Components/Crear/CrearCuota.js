...
export const CrearCuota = () => {
  ...
  const MostrarAlerta = () => {
    Swal.fire({
      ...
      window.location.href = "/cuotasListado";
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${apiRest}/setting/cuotas`, {
        method: "POST",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ numero, descripcion, interes, 'tipo_cuota': tipoCuota }),
      });
