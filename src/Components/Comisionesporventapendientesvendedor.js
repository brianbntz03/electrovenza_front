...
export const Comisionesporventapendientesvendedor = () => {
  ...
  const updateCuentaCorriente = async (vededorId) => {
    try {
      const response = await fetch(
        `${apiRest}/cuenta-corriente/get-by-vendedor/${vededorId}`
      );
      if (response.ok) {
  ...
  const liquidarComisionesVentas = async () => {
    try {
      ...
      const responseVentas = await fetch(
        `${apiRest}/ventas/comisiones/marcar-comisiones-como-pagadas/`,
        {
          method: "POST",
          headers: {
      ...
      if (!responseVentas.ok) {
        throw new Error(
          `Error al pagar las comisiones por ventas: ${responseVentas.status}`
        );
      }
  ...
  const liquidarComisionesCreditos = async () => {
    try {
      ...
      const responseCreditos = await fetch(
        `${apiRest}/credito/comisiones/marcar-comisiones-como-pagadas/`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!responseCreditos.ok) {
        throw new Error(
          `Error al pagar las comisiones por creditos: ${responseCreditos.status}`
        );
      }
  ...
  const registrarLiquidacionCobranza = async () => {
    try {
      if (!idVendedor) {
        FlashMessage(
          "Registrar Liquidacion de Cobranza",
          "Debe seleccionar un vendedor",
          2000,
          "warning"
        );
      ...
      const registroLiquidacionCobranza = await fetch(
        `${apiRest}/cuenta-corriente-movimiento/liquidar-cobranza`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!registroLiquidacionCobranza.ok) {
        throw new Error(
          `Error al registrar la liquidación de cobranza: ${registroLiquidacionCobranza.status}`
        );
      }
      ...
      FlashMessage(
        "Registro de liquidación de cobranza",
        "La liquidación fue registrada",
        2000,
        "success"
      );
    } catch (error) {
  ...
  const ComisionesPorVendedor = async (vededorId) => {
    ...
    try {
      const response = await fetch(
        `${apiRest}/cuenta-corriente/last-ten-movements-by-vendedor/${vededorId}`
      );
      if (response.ok) {
            ...
            <tbody>
              ...
              {comisionesCreditos.map((comision, index) => (
                <tr key={index}>
                  ...
                  <td>{comision.cuota_credito.credito.articulo?.nombre}</td>
                  <td>{comision.monto}</td>
                  <td>{comision.cuota_venta.monto_cobrado}</td>
                </tr>
              ))}
